    ...
    # Handle Windows system
        ...
        if routes:
            ...
            if primary_interface_ip and adapters:
                ...
                # If any of these is None, try the WMI approach
                if None in (subnet_mask, dhcp_server_ip, dns_server_ip) or not default_gateways:
                    wmi_dhcp, wmi_dns, wmi_gateway, wmi_subnet = get_windows_network_info_wmi()
                    if not subnet_mask and wmi_subnet:
                        subnet_mask = wmi_subnet
                    if not dhcp_server_ip and wmi_dhcp:
                        dhcp_server_ip = wmi_dhcp
                    if not dns_server_ip and wmi_dns:
                        dns_server_ip = wmi_dns
                    if not default_gateways and wmi_gateway:
                        default_gateways = [wmi_gateway]
    
    # Handle any other OS
    else:
        return {
            "dhcp_server_ip": "Non disponible (système non supporté)",
            "dns_server_ip": "Non disponible (système non supporté)",
            "default_gateway(s)": "Non disponible (système non supporté)",
            "subnet_mask": "Non disponible (système non supporté)"
        }
    
    # Prepare and return the dictionary of network configuration values
    if system == 'Linux':
        return {
            "dhcp_server_ip": dhcp_server_ip if dhcp_server_ip else "Non disponible",
            "dns_server_ip": dns_server_ip if dns_server_ip else "Non disponible",
            "default_gateway(s)": ", ".join(default_gateways) if default_gateways else "Non disponible",
            "subnet_mask": subnet_mask if subnet_mask else "Non disponible"
        }
    else:  # system == 'Windows'
        return {
            "dhcp_server_ip": dhcp_server_ip if dhcp_server_ip else "Non disponible",
            "dns_server_ip": dns_server_ip if dns_server_ip else "Non disponible",
            "default_gateway(s)": ", ".join(default_gateways) if isinstance(default_gateways, list) else default_gateways,
            "subnet_mask": subnet_mask if subnet_mask else "Non disponible"
        }

# Testing function to print all details to console
...
# Self-test - uncomment this line to run when directly executed
