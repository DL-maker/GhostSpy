...
def create_pdf_with_data(file_name, data):
    ...
    content.append(Spacer(1, 12))
    add_section(content, "Network Activity", data['network_activity'], styles['Normal_Left'], is_dict=True)
    add_section(content, "Connection_information", data['connection_information'], styles['Normal_Left'], is_dict=True)
    content.append(LineChart())
    content.append(Spacer(1, 12))
    content.append(Paragraph("Data usage in MB / 5 min", styles['Normal_Left']))
    content.append(Spacer(1, 12))
    add_section(content, "Host Characteristics", data['host_characteristics'], styles['Normal_Left'], is_dict=True)
    add_section(content, "Localization and Environment", data['localization_and_environment'], styles['Normal_Left'], is_dict=True)
    add_section(content, "Network Configuration", data['network_configuration'], styles['Normal_Left'], is_dict=True)
    doc.build(content)
