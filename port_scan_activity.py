...
# HTTP/HTTPS-related ports
HTTP_PORTS = {80, 443, 8080, 8000, 8081, 8888, 5500}

# Ensemble des connexions déjà enregistrées pour éviter les doublons
seen_connections = set()
# Cache для доменных имен, чтобы избежать повторных DNS-запросов
domain_cache = {}
...
def get_domain_name(ip):
    ...
    try:
        # Попытка выполнить обратный DNS-запрос
        domain = socket.gethostbyaddr(ip)[0]
        domain_cache[ip] = domain
        return domain
    except (socket.herror, socket.gaierror):
        # Если не удалось получить доменное имя, возвращаем просто IP
        domain_cache[ip] = ip
        return ip
...
def log_connection(ip, port, laddr, raddr, pid, proc_name):
    ...
    # Если это HTTP или HTTPS, добавляем доменное имя
    domain_info = ""
    if port in HTTP_PORTS or service in ["HTTP", "HTTPS", "HTTP Proxy", "HTTP Alternative"]:
        domain_name = get_domain_name(raddr.ip)
        if domain_name != raddr.ip:  # Если доменное имя получено успешно
            domain_info = f" [{domain_name}]"
    
    log_line = (f"[{timestamp}] Service: {service} | "
               f"{laddr.ip}:{laddr.port} -> {raddr.ip}:{raddr.port}{domain_info} | "
               f"PID: {pid} ({proc_name})\n")
...
def monitor_ports():
    while True:
        ...
        # Ограничение размера кеша доменных имен
        if len(domain_cache) > 1000:
            domain_cache.clear()
