...
def get_network_configuration():
    ...
    # Handle Windows system
        ...
        if routes:
            ...
            if primary_interface_ip and adapters:
                ...
                default_gateways = ", ".join(get_windows_default_gateways(routes))
            else:
                # Fallback to WMI if standard approach failed
                dhcp_server, dns_servers, default_gateway, subnet_mask_wmi = get_windows_network_info_wmi()
                
                if dhcp_server:
                    dhcp_server_ip = dhcp_server
                if dns_servers:
                    dns_server_ip = dns_servers
                if default_gateway:
                    default_gateways = default_gateway
                if subnet_mask_wmi:
                    subnet_mask = subnet_mask_wmi
    else:
        # Unsupported OS
        return {
            "dhcp_server_ip": "Non disponible",
    ...
    # Sanitize output
    if not subnet_mask or subnet_mask == "0.0.0.0":
        subnet_mask = "Non disponible"
    if not dhcp_server_ip or dhcp_server_ip == "0.0.0.0":
        dhcp_server_ip = "Non disponible"
    if not dns_server_ip or dns_server_ip == "0.0.0.0":
        dns_server_ip = "Non disponible"
    if not default_gateways or default_gateways == "0.0.0.0":
        default_gateways = "Non disponible"
